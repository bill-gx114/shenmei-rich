...
const TWEAKS_STYLE = `
  .twk-fab{position:fixed;right:16px;bottom:16px;z-index:2147483645;
    background:rgba(0,0,0,.78);color:#fff;border:0;border-radius:999px;
    padding:9px 16px;font:12px/1 ui-sans-serif,system-ui,-apple-system,sans-serif;
    box-shadow:0 6px 20px rgba(0,0,0,.35);cursor:pointer;letter-spacing:.04em}
...
export function TweaksPanel({ title = 'Tweaks', children }: { title?: string; children: ReactNode }) {
  ...
  return (
    <>
      ...
      {!open && (
        <button className="twk-fab" onClick={() => setOpen(true)} aria-label="打开设置">
          ⚙ 展厅设置
        </button>
      )}
      {open && (
